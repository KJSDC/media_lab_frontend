...
const ItemImage = ({
  ...
  const src =
    imageB64 || (imageUrl ? `http://localhost:5000${imageUrl}` : null);
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (itemsRes.data.success) {
        const activeItems = itemsRes.data.items.filter(
          (i) => i.is_active !== false,
        );
        setItems(activeItems);
        setChartData(buildWeekActivity(activeItems));
      }
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-6 p-3 bg-gray-50 rounded-xl border border-gray-100">
              <div className="w-12 h-12 rounded-xl bg-white border border-gray-100 overflow-hidden flex-shrink-0 flex items-center justify-center">
                <ItemImage
                  imageB64={selectedRow.imageB64}
                  name={selectedRow.itemName}
                />
              </div>
              <div>
